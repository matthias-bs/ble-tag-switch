# Plan: Step-wise ESP32 Implementation for ATUVOS Tag Detection with Verification

Build and verify reliable detection of two ATUVOS air-tag pro devices through a guided, multi-stage ESP32 sketch sequence that captures fingerprints, verifies MAC rotation, and validates relay control logic.
...
### Discovery Phase
- [ ] Both tags identified with unique characteristics
- [ ] Model IDs captured
- [ ] Device names match tag labels (Atuvos-XXXX)

### Fast Pair Analysis
- [ ] Frame types transition correctly (discoverable → non-discoverable)
- [ ] Bloom filter hashes differ between tags
- [ ] Manufacturer data patterns consistent

### MAC Rotation
- [ ] MAC address changes observed (~every 17 minutes)
- [ ] Bloom filter hash remains constant across MAC changes
- [ ] RSSI stable from fixed position (±5 dBm variance acceptable)

### Multi-Tag Discrimination
- [ ] Each tag correctly identified despite MAC rotation
- [ ] Tags distinguishable even at same distance
- [ ] No cross-identification or confusion

### Find Hub Network
- [ ] 0xFEAA advertisements captured post-pairing
- [ ] Ephemeral IDs unique per tag
- [ ] Advertising interval ~2 seconds

### RSSI Calibration
- [ ] RSSI vs distance curve documented
- [ ] Thresholds with ≥13 dBm hysteresis
- [ ] Both tags behave consistently

### Relay Logic
- [ ] Activation/deactivation at correct RSSI thresholds
- [ ] 2-second debounce prevents flickering
- [ ] Handles simultaneous multi-tag detection

### Hardware Integration
- [ ] Relay clicks when tag detected
- [ ] Relay releases when tag out of range
- [ ] Reliable operation for 24+ hours

---

## Sketch Output Format Design

All sketches use consistent human-readable format:

```
╔═══════════════════════════════════════════════════════════╗
║           ATUVOS TAG DETECTION - STAGE 2                 ║
║          Fast Pair Service Analysis                      ║
╚═══════════════════════════════════════════════════════════╝

[SCAN ACTIVE] 45 seconds elapsed | 143 advertisements received

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
TAG DETECTED: AC:DE:48:12:34:56
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Mode:              PAIRING (Discoverable)
  ...
  Manufacturer Data: 0x0A 0x1B 0x2C 0x3D 0x4E
  Full Payload:      02 01 06 03 03 2C FE ...
...
→ Move this tag away from ESP32
→ Bring the second tag (Tag-2) into range
...
- 2 × ATUVOS air-tag pro devices
- USB cable for programming/serial monitoring
- (Stage 8 only) Relay module + wire connectors
...
|-------|----------|----------|
| 1 | 5-10 min | Discovery & baseline |
| 2 | 10-15 min | Fast Pair analysis + mode switching |
| 3 | 25-30 min | MAC rotation verification (wait for rotation) |
| 4 | 10-15 min | Multi-tag discrimination tests |
| 5 | 10-15 min | Find Hub Network capture |
| 6 | 15-20 min | RSSI calibration at multiple distances |
| 7 | 10-15 min | Relay logic simulation |
| 8 | 5-10 min | Hardware integration + final test |
| **Total** | **90-130 min** | **Complete verification** |

---

## Further Considerations

1. **Tag State Persistence** - Do RSSI/Bloom filter patterns remain stable after multiple pairing cycles, or should re-calibration be performed?

2. **Environmental Factors** - Should testing include: obstacles between ESP32/tags, different materials, other RF interference, temperature variations?

3. **Find My Device Network Integration** - After pairing via Android, verify tag is reporting to Find My Device network (may require Google account verification to confirm from ESP32 perspective).

4. **Error Handling** - How should ESP32 respond to: tag battery low/dead, tag firmware update, lost GATT connection attempts, or corrupted BLE advertisements?

5. **Storage Approach** - Store fingerprints hardcoded in sketches, or implement EEPROM/SPIFFS-based configuration for easier multi-tag management?
